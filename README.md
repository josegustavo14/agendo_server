...
Autenticado. Retorna todas as avaliações recebidas por um profissional. A média é atualizada automaticamente.
...
#### `GET /appointments/active` — Agendamentos ativos (Home)
Retorna agendamentos com status `PENDING` ou `APPROVED`. Ideal para exibir na home do app — são os que precisam de atenção ou ação. Ordenados por data do agendamento.
...
[
  {
    "id": 1,
    "professional": { "id": 1, "name": "João Silva" },
    "client": { "id": 2, "name": "Ana Costa" },
    "services": [{ "id": 1, "name": "Instalação Elétrica", "price": 150.00 }],
    "totalAmount": 150.00,
    "scheduleDate": "2026-04-10T14:00:00",
    "requestDate": "2026-04-08T10:00:00",
    "status": "APPROVED"
  }
]
```
...
#### `GET /appointments/archive` — Histórico de agendamentos
Retorna agendamentos encerrados com status `COMPLETED`, `CANCELLED` ou `REJECTED`. Exibir em seção separada do app.

**Resposta 200:** mesma estrutura de `/active`, com status finais.
...
#### `GET /appointments/{id}/timeline` — Timeline de um agendamento
Retorna a trilha cronológica de mudanças de status de um agendamento específico. Apenas participantes têm acesso.
...
[
  {
    "previousStatus": null,
    "newStatus": "PENDING",
    "changedBy": { "id": 2, "name": "Ana Costa" },
    "changedAt": "2026-04-08T10:00:00"
  },
  {
    "previousStatus": "PENDING",
    "newStatus": "APPROVED",
    "changedBy": { "id": 1, "name": "João Silva" },
    "changedAt": "2026-04-08T11:30:00"
  },
  {
    "previousStatus": "APPROVED",
    "newStatus": "COMPLETED",
    "changedBy": { "id": 1, "name": "João Silva" },
    "changedAt": "2026-04-08T15:00:00"
  }
]
```
...
#### `GET /appointments/professional` — Listar agendamentos como profissional
Atalho para retornar apenas os agendamentos onde o usuário autenticado é o profissional.

---

#### `PATCH /appointments/{id}/approve` — Aprovar agendamento
Apenas o **profissional** do agendamento pode aprovar. O status deve estar em `PENDING`.
...
Apenas o **profissional** pode rejeitar. O status deve estar em `PENDING`.
...
Cliente **ou** profissional pode cancelar. O status deve estar em `APPROVED`.
...
Apenas o **profissional** pode marcar como concluído. O status deve estar em `APPROVED`.
...
|--------|----------|
| `PENDING` | Home (ativos) |
| `APPROVED` | Home (ativos) |
| `REJECTED` | Histórico (archive) |
| `CANCELLED` | Histórico (archive) |
| `COMPLETED` | Histórico (archive) |
...
| POST | `/appointments` | ✅ | Criar agendamento |
| GET | `/appointments` | ✅ | Listar todos os agendamentos |
| GET | `/appointments/active` | ✅ | Ativos: PENDING + APPROVED (home) |
| GET | `/appointments/archive` | ✅ | Encerrados: COMPLETED + CANCELLED + REJECTED |
| GET | `/appointments/{id}` | ✅ | Buscar agendamento por ID |
| GET | `/appointments/{id}/timeline` | ✅ | Trilha de mudanças de status |
| GET | `/appointments/professional` | ✅ | Listar agendamentos como profissional |
...
Na inicialização, o `DataSeeder` limpa e repopula o banco com dados de exemplo. Todos os usuários têm senha padrão: `123456`.

### Profissões
Eletricista, Desenvolvedor, Encanador, Designer, Personal Trainer

### Usuários

| Nome | Email | Role |
|------|-------|------|
| João Silva | joao@email.com | PROFESSIONAL (Eletricista) |
| Maria Oliveira | maria@email.com | PROFESSIONAL (Desenvolvedora) |
| Carlos Santos | carlos@email.com | PROFESSIONAL (Encanador) |
| Ana Costa | ana@email.com | CLIENT |
| Pedro Almeida | pedro@email.com | CLIENT |

### Agendamentos

| # | Profissional | Cliente | Serviços | Total |
|---|-------------|---------|----------|-------|
| 1 | João | Ana | Instalação Elétrica + Inspeção Elétrica | R$ 230,00 |
| 2 | Maria | Pedro | Desenvolvimento Web + Debug e Correção de Bugs | R$ 380,00 |
| 3 | João | Pedro | Manutenção Elétrica | R$ 120,00 |
| 4 | Maria | Ana | Desenvolvimento de API + Testes e QA | R$ 320,00 |
| 5 | Carlos | Ana | Reparo Hidráulico + Troca de Sifão | R$ 240,00 |

### Avaliações

| Profissional | Cliente | Nota | Comentário |
|-------------|---------|------|------------|
| João | Ana | ⭐⭐⭐⭐⭐ | "Ótimo profissional, muito pontual e organizado!" |
| João | Pedro | ⭐⭐⭐⭐ | "Bom trabalho, resolveu o problema rapidamente." |
| Maria | Pedro | ⭐⭐⭐⭐⭐ | "Excelente desenvolvedora, entregou antes do prazo!" |
| Maria | Ana | ⭐⭐⭐⭐ | "Muito competente, recomendo." |
| Carlos | Ana | ⭐⭐⭐ | "Resolveu o problema, mas demorou mais do esperado." |
